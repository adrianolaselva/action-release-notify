...
func main() {
	...
	tag := ""
	if value, ok := syscall.Getenv("TAG"); ok {
		tag = value
	}

	endpoint := ""
	if value, ok := syscall.Getenv(EnvSlackWebhook); ok {
		endpoint = value
	}

	if _, ok := syscall.Getenv(EnvSlackMessage); !ok {
	...
	tagResp, _, _ := client.Repositories.GetReleaseByTag(ctx, owner, repository, tag)
	...
	if err := os.Setenv(EnvSlackMessage, message.String()); err != nil {
		fmt.Fprintln(os.Stderr, "Failet to set message")
		os.Exit(2)
	}
	...
	if err := sendNotication(endpoint, msg); err != nil {
		fmt.Fprintf(os.Stderr, "Error sending message: %s\n", err)
		os.Exit(2)
	}
}

func sendNotication(endpoint string, msg Webhook) error {
